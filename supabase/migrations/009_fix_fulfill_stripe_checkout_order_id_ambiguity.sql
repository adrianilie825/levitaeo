-- Levitaeo: fix PL/pgSQL ambiguity between RETURNS TABLE output columns and table columns.
-- fulfill_stripe_checkout exposes order_id / entitlement_id as output parameters, which
-- conflicted with order_items.order_id in ON CONFLICT (order_id, product_slug).
...
-- Same ambiguity class: record_stripe_checkout_status also RETURNS TABLE (order_id uuid).
